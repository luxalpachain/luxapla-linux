...
PROJECT="$BUILDDIR/../../luxalpabuilds/project-$1"
if [ ! -d "$BUILDDIR/../../luxalpabuilds" ]; then
    mkdir -p $BUILDDIR/../../luxalpabuilds
fi
CURRENT_OS=$1
...
if [ ! -d $PROJECT ]; then
	...
	cordova create ../../luxalpabuilds/project-$1 org.luxalpa.wallet LuxAlpa-1.1.1
	checkOK
...
#  checkOK
	...
	cordova plugin add cordova-plugin-customurlscheme --variable URL_SCHEME=luxalpa
	checkOK
...
if $DBGJS
then
	echo "${OpenColor}${Green}* Generating luxalpa bundle (debug js)...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova
	checkOK
else
	echo "${OpenColor}${Green}* Generating luxalpa bundle...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova-prod
	checkOK
fi
...
cp node_modules/luxalpacore/initial.luxalpa.sqlite $PROJECT/www
cp node_modules/luxalpacore/initial.luxalpa-light.sqlite $PROJECT/www
checkOK
...
if [ $CURRENT_OS == "ANDROID" ]; then
	...
	cat $BUILDDIR/android/android.css >> $PROJECT/www/css/luxalpa.css
