...
angular.module('copayApp.controllers').controller('wordsController', function ($rootScope, $scope, $timeout, profileService, go, gettext, gettextCatalog, confirmDialog, notification, $log, isCordova, storageService,configService,uxLanguage) {


	var lg = uxLanguage.getCurrentLanguage();
    ...
    /*if(lg==='zh_CN'){
        msg = gettextCatalog.getString('您确定要删除备份文档吗？');
        successMsg = gettextCatalog.getString('备份文档已删除');
    }else if(lg==='ko_KR'){
        msg = gettextCatalog.getString('백업 문서를 삭제 하시겠습니까?');
        successMsg = gettextCatalog.getString('백업 문서가 삭제되었습니다.');
    }else if(lg==='ja_JP'){
        msg = gettextCatalog.getString('백バックアップドキュメントを削除してもよろしいですか？');
        successMsg = gettextCatalog.getString('バックアップ文書が削除されました');
    }
*/
	var self = this;
	self.show = false;
