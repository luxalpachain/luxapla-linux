...
var constants = require('luxalpacore/constants.js');
var eventBus = require('luxalpacore/event_bus.js');
var breadcrumbs = require('luxalpacore/breadcrumbs.js');
var ValidationUtils = require('luxalpacore/validation_utils.js');
var db = require('luxalpacore/db.js');
...
angular.module('copayApp.controllers')
    .controller('walletHomeController', function ($scope, $rootScope, $timeout, $filter, $modal, $log, notification, isCordova, profileService, lodash, configService, storageService, gettext, gettextCatalog, nodeWebkit, addressService, confirmDialog, animationService, addressbookService, correspondentListService, newVersion, autoUpdatingWitnessesList) {
        ...
        var chatStorage = require('luxalpacore/chat_storage.js');
        ...
        var disablePaymentRequestListener = $rootScope.$on('paymentRequest', function (event, address, amount, asset, recipient_device_address) {
            ...
            if (form.address && form.address.$invalid && !self.blockUx) {
                ...
                self.resetForm();
                self.error = gettext('Could not recognize a valid LuxAlpaChain QR Code');
            }
        });
        ...
        $scope.openSharedAddressDefinitionModal = function (address) {
            ...
            var ModalInstanceCtrl = function ($scope, $modalInstance) {
                ...
                var walletDefinedByAddresses = require('luxalpacore/wallet_defined_by_addresses.js');
            ...
            var ModalInstanceCtrl = function ($scope, $modalInstance) {
                ...
                $scope.submitForm = function (form) {
                    ...
                    $timeout(function () {
                        ...
                        console.log("---------",amount);
                        console.log("---------",amountInSmallestUnits);
                        $scope.asset_param = (asset === 'base') ? '' : '&asset=' + encodeURIComponent(asset);
                        console.log("---------",(asset === 'base') ? '' : '&asset=' + encodeURIComponent(asset));
                    }, 1);
                };
        ...
        function claimTextCoin(mnemonic, addr) {
            var wallet = require('luxalpacore/wallet.js');
        ...
        function getShareMessage(amount, mnemonic, asset) {
            ...
            return {
                message: "Here is your link to receive " + amount + " " + asset + usd_amount_str + ": https://lux.one/openapp.html#textcoin?" + mnemonic,
                subject: "LuxAlpaChain user beamed you money"
            }
        }
        ...
        this.submitPayment = function () {
            ...
            db.query(
                "SELECT unit FROM assets where assets_name=?",[asset],function (rows) {
                    if(rows.length==1){asset = rows[0].unit}
                }
            )
            //alert(asset);
                ...
                profileService.requestTouchid(function (err) {
                    ...
                    if (self.binding) {
                        if (isTextcoin) {
                            delete self.current_payment_key;
                            indexScope.setOngoingProcess(gettext('sending'), false);
                            return self.setSendError("you can send bound payments to luxalpa adresses only");
                        }
                        if (!recipient_device_address)
                            throw Error('recipient device address not known');
                        var walletDefinedByAddresses = require('luxalpacore/wallet_defined_by_addresses.js');
                        var walletDefinedByKeys = require('luxalpacore/wallet_defined_by_keys.js');
                        var my_address;
                        ...
                        fc.sendMultiPayment(opts, function (err, unit, mnemonics) {
                            ...
                            if (err) {
                                if (typeof err === 'object') {
                                    ...
                                    err = err.substring(err.indexOf("from")+4, err.indexOf("for")) + gettextCatalog.getString(err.substr(0,err.indexOf("from"))) + gettextCatalog.getString(". It needs at least")  + parseInt(err.substring(err.indexOf("for")+3, err.length)) / 1000000 + "LAC";
                                self.setSendError(err);
                                return false;
                            }
                            ...
                            if (recipient_device_address) { // show payment in chat window
                                ...
                                if (binding && binding.reverseAmount) { // create a request for reverse payment
                                    ...
                                    var paymentRequestCode = 'luxalpa:' + my_address + '?amount=' + binding.reverseAmount + '&asset=' + encodeURIComponent(binding.reverseAsset);
        ...
        this.submitData = function () {
            var objectHash = require('luxalpacore/object_hash.js');
            var fc = profileService.focusedClient;
            var value = {};
            var app;
        ...
        this.indexchangeunit = function () {
            this.bSendAll = false;
            var selectselect = document.getElementById('indexchangeunits');
            var selectedcurrency = selectselect.options[indexScope.assetIndex].getAttribute('data-totalamount');
            // console.log("selectedText"+selectedText);
            var selectunit = selectselect.options[indexScope.assetIndex].getAttribute('data-currency');
            var selectcurreyname = '';
            // console.log(indexScope.assetIndex);
            selectcurreyname = 	selectunit;
            // if(indexScope.assetIndex == 0){
            //    selectcurreyname = 	'LAC';
            // }else if(indexScope.assetIndex == 1){
            //    selectcurreyname = 	'black';
            // }else if(indexScope.assetIndex == 2){
            //    selectcurreyname = 	'ddd';
            // }
            // console.log(indexScope.assetIndex)
            // console.log(selectcurreyname)
            angular.element(document.getElementsByClassName('changeunit')).text(selectcurreyname);
            angular.element(document.getElementsByClassName('changeunit')).text(selectcurreyname);
            angular.element(document.getElementById('changunitotal')).text(selectedcurrency);
        }
        ...
        this.openTxModal = function (btx) {
            ...
            var ModalInstanceCtrl = function ($scope, $modalInstance) {
                ...
                $scope.openInExplorer = function () {
                    ...
                    var url = 'https://' + testnet + 'explorer.luxalpa.org/#' + btx.unit;
                ...
                $scope.reSendPrivateMultiSigPayment = function () {
                    var indivisible_asset = require('luxalpacore/indivisible_asset');
                    var wallet_defined_by_keys = require('luxalpacore/wallet_defined_by_keys');
                    var walletDefinedByAddresses = require('luxalpacore/wallet_defined_by_addresses');
                    var fc = profileService.focusedClient;
        ...
        this.showCorrespondentListToReSendPrivPayloads = function (btx) {
            ...
            var ModalInstanceCtrl = function ($scope, $modalInstance, $timeout, go, notification) {
                ...
                $scope.sendPrivatePayments = function (correspondent) {
                    var indivisible_asset = require('luxalpacore/indivisible_asset');
                    var wallet_general = require('luxalpacore/wallet_general');
                    indivisible_asset.restorePrivateChains(btx.asset, btx.unit, btx.addressTo, function (arrRecipientChains, arrCosignerChains) {
