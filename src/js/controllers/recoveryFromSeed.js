...
angular.module('copayApp.controllers').controller('recoveryFromSeed',
	function ($rootScope, $scope, $timeout, profileService, go, gettext, gettextCatalog, confirmDialog, notification, $log, isCordova,storageService) {

		var async = require('async');
		var conf = require('luxalpacore/conf.js');
		var wallet_defined_by_keys = require('luxalpacore/wallet_defined_by_keys.js');
		var objectHash = require('luxalpacore/object_hash.js');
		try{
			var ecdsa = require('secp256k1');
		}
		catch(e){
			var ecdsa = require('luxalpacore/node_modules/secp256k1' + '');
		}
		var Mnemonic = require('bitcore-mnemonic');
		var Bitcore = require('bitcore-lib');
		var db = require('luxalpacore/db.js');
		var network = require('luxalpacore/network');
		var myWitnesses = require('luxalpacore/my_witnesses');
...
// 定义提示框内容
		...
		function scanForAddressesAndWalletsInLightClient(mnemonic, cb) {
			...
			function checkAndAddCurrentAddresses(is_change) {
				...
				var arrTmpAddresses = [];
					arrTmpAddresses.push(objectHash.getChash160(["sig", {"pubkey": wallet_defined_by_keys.derivePubkey(xPubKey, 'm/0'/'0')}]));
		...
		function cleanAndAddWalletsAndAddresses(assocMaxAddressIndexes) {
			var device = require('luxalpacore/device');
					...
					profileService.replaceProfile(self.xPrivKey.toString(), self.inputMnemonic, myDeviceAddress, function () {
						...
						createWallets(arrWalletIndexes, function () {
							createAddresses(assocMaxAddressIndexes, function () {
								self.scanning = false;
								$rootScope.$emit('Local/ShowAlertdirs', arrWalletIndexes.length + " wallets recovered, please restart the application to finish.", 'fi-check', function () {
...
// 更改代码   修改后恢复（没有交易）
		...
		self.recoveryForm = function() {
            ...
            //self.inputMnemonic = "crunch hip february casual age course olive surprise kind desert name december";
            if (!profileService.focusedClient.credentials.mnemonic) {
