var BLACKBYTES_ASSET = require('luxalpacore/constants').BLACKBYTES_ASSET;
var balances = require('luxalpacore/balances');
...
function initWallet() {
	...
	function loadCompleteClient(showClient) {
		self._bLuxAlpaCoreLoaded = false; //"fix" : Looks like you are loading multiple copies of luxalpa core, which is not supported. Running 'npm dedupe' might help.
		...
		angularJs.onload = function() {
			var luxalpaJS = document.createElement('script');
			luxalpaJS.src = 'luxalpa.js';
			body.appendChild(luxalpaJS);
			luxalpaJS.onload = function() {
