...
angular
	...
	.config(function (historicLogProvider, $provide, $logProvider, $stateProvider, $urlRouterProvider, $compileProvider, $qProvider) {
		...
		$stateProvider
            ...
            .state('restoreWallet', {
                ...
                views: {
                    'main': {
                        templateUrl: 'views/restoreWallet.html',
                    }
                }
            });
		...
		$stateProvider
            ...
            .state('correspondentDevices.chataddCorrespondentDevice', {
                url: '/addchat',
                needProfile: true,
                views: {
                    'dialog': {
                        templateUrl: 'views/chataddCorrespondentDevice.html'
                    },
                }
            })
            ...
            .state('correspondentDevices.chataddCorrespondentDevice.inviteCorrespondentDevice', {
                url: '/invitechat',
                walletShouldBeComplete: false,
                needProfile: true,
                views: {
            ...
            .state('correspondentDevices.chataddCorrespondentDevice.acceptCorrespondentInvitation', {
                url: '/acceptCorrespondentInvitationchat',
                walletShouldBeComplete: false,
                needProfile: true,
                views: {
                    'dialog@correspondentDevices': {
	...
	.run(function ($rootScope, $state, $log, uriHandler, isCordova, profileService, storageService, $timeout, nodeWebkit, uxLanguage, animationService) {
		...
		// 	});
		// 	try {
		// 		nativeMenuBar.createMacBuiltin("LuxAlpa");
		// 	} catch (e) {
			...
			if (!profileService.profile && toState.needProfile) {
				...
				profileService.loadAndBindProfile(function (err) {
					...
					if (err) {
						...
						// 用于解决创建钱包生成口令后，没有进行口令校验的情况下，关闭app再次打开app可以跳过校验直接进入首页的bug
                        storageService.gethaschoosen(function (err, val) {
                            if (val == 1) {
                            	// 状态标识1：进入创建钱包页面
                                $log.debug('No choosen... redirecting');
                                $state.transitionTo('createWallet');
                            }
                            else {
                            	//否则，进入首页
