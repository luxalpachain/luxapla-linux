...
angular.module('copayApp.services')
    .factory('profileService', function profileServiceFactory($rootScope, $location, $timeout, $filter, $log, lodash, storageService, bwcService, configService, pushNotificationsService, isCordova, gettext, gettextCatalog, nodeWebkit, uxLanguage) {
        ...
        root.formatAmountWithUnitNoSeparators = function (amount, asset, opts) {
            return root.formatAmountWithUnit(amount, asset, opts).replace(/,/g, "");
        };
        ...
        root._createNewProfile = function (opts, cb) {
            ...
            root._seedWallet({}, function (err, walletClient) {
                if (err)
                    return cb(err);
                var config = configService.getSync();
                require('luxalpacore/wallet.js'); // load hub/ message handlers
                var device = require('luxalpacore/device.js');
                var tempDeviceKey = device.genPrivKey();
                ...
                var walletName = gettextCatalog.getString('LuxAlpa');     //默认钱包名称
        ...
        root._addWalletClient = function (walletClient, opts, cb) {
            ...
            if (w)
                return cb(gettext('Wallet already in LuxAlpa' + ": ") + w.walletName);
        ...
        root._normalizeMnemonic = function (words) {
            var isJA = words.indexOf('\u3000') > -1;    //判断是否存在中文全角空格字符
            var wordList = words.split(/[\u3000\s]+/);  //根据一个或者多个中文全角空格或空白字符进行分隔单词

            return wordList.join(isJA ? '\u3000' : ' ');    //用空格作为连接符对单词列表进行拼接
        };
