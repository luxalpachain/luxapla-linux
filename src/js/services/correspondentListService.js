...
angular.module('copayApp.services').factory('correspondentListService', function($state, $rootScope, $sce, $compile, configService, storageService, profileService, go, lodash, $stickyState, $deepStateRedirect, $timeout, gettext) {
        ...
        if (bIncoming) {
            ...
            if ($rootScope.newMessagesCount[peer_address] == 1 && (!$state.is('correspondentDevices.correspondentDevice') || root.currentCorrespondent.device_address != peer_address)) {
                root.messageEventsByCorrespondent[peer_address].push({
                    bIncoming: false,
                    message: '<span style=\'color:#fff;background-color:#cad1d9;padding: 4px 10px;border-radius: 2px;\' translate>new messages</span>',
                    type: 'system',
    ...
    var payment_request_regexp = /\[.*?\]\(luxalpa:([0-9A-Z]{32})\?([\w=&;+%]+)\)/g; // payment description within [] is ignored
    ...
    function highlightActions(text, arrMyAddresses){
        return text.replace(/\b[2-7A-Z]{32}\b(?!(\?(amount|asset|device_address)|"))/g, function(address){
            ...
            // 	+'" class="f-dropdown" style="left:0px" data-dropdown-content><li><a ng-click="sendPayment(\''+address+'\')">'+gettext('Pay to this address')+'</a></li><li><a ng-click="offerContract(\''+address+'\')">'+gettext('Offer a contract')+'</a></li></ul>';
            // return '<a dropdown-toggle="#pop'+address+'">'+address+'</a><ul id="pop'+address
            //     +'" class="f-dropdown" style="left:0px" data-dropdown-content><li><a ng-click="sendPayment(\''+address+'\')">'+gettext('Pay to this address')+'</a></li></ul>';
    ...
    function parsePaymentRequestQueryString(query_string){
        var URI = require('luxalpacore/uri.js');
            ...
            walletGeneral.readMyAddresses(function(arrMyAddresses){
                ...
                for (var i in messages) {
                    ...
                    if (last_msg_ts && last_msg_ts.getDay() != msg_ts.getDay()) {
                        messageEvents.unshift({type: 'system', bIncoming: false, message: "<span style='color:#C4C4C4;background-color:#F9FAFC;padding: 4px 13px;font-family:pfmedium;font-size:13px;'>" + last_msg_ts.toDateString() + "</span>", timestamp: Math.floor(msg_ts.getTime() / 1000)});
                    }
                    last_msg_ts = msg_ts;
                ...
                if (historyEndForCorrespondent[correspondent.device_address] && messageEvents.length > 1) {
                    messageEvents.unshift({type: 'system', bIncoming: false, message: "<span style='color:#C4C4C4;background-color:#F9FAFC;padding: 4px 13px;font-family:pfmedium;font-size:13px;'>" + (last_msg_ts ? last_msg_ts : new Date()).toDateString() + "</span>", timestamp: Math.floor((last_msg_ts ? last_msg_ts : new Date()).getTime() / 1000)});
                }
                $timeout(function(){
                    $rootScope.$digest();
                });
    ...
    function checkAndInsertDate(messageEvents, message) {
        ...
        if (last_msg_ts.getDay() != msg_ts.getDay()) {
            messageEvents.push({type: 'system', bIncoming: false, message: "<span style='color:#C4C4C4;background-color:#F9FAFC;padding: 4px 13px;font-family:pfmedium;font-size:13px;'>" + msg_ts.toDateString() + "</span>", timestamp: Math.floor(msg_ts.getTime() / 1000)});
        }
    }
    ...
    function parseMessage(message) {
        switch (message.type) {
            case "system":
                ...
                message.message = "<span style='color:#C4C4C4;background-color:#F9FAFC;padding: 4px 13px;font-family:pfmedium;font-size:13px;' translate>chat record</span><b style=\"font-size:0;margin:0;padding:0;border:none;\" dropdown-toggle=\"#recording-drop\">" + (message.message.state ? "ON" : "OFF") + "</b><span style=\"font-size:0;margin:0;padding:0;border:none;\" class=\"padding\"></span>";
