module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		exec: {
			...
			osx64: {
				command: '../luxalpabuilds/build-osx.sh osx64'
			},
			osx32: {
				command: '../luxalpabuilds/build-osx.sh osx32'
			}
		},
		watch: {
			options: {
		...
		concat: {
			...
			js: {
				...
				dest: 'public/luxalpa.js'
			},
			css: {
				src: ['src/css/*.css'],
				dest: 'public/css/luxalpa.css'
			},
			foundation: {
				src: [
		...
		uglify: {
			...
			prod: {
				files: {
					'public/luxalpa.js': ['public/luxalpa.js'],
		...
		copy: {
			...
			node_modules:{
				expand: true,
				src: 'node_modules/**',
				dest: '../luxalpabuilds/LuxAlpa/win64/'
			},
				...
				files: [
					{
						...
						dest: '../luxalpabuilds/LuxAlpa/linux32/',
						flatten: true,
						filter: 'isFile',
						options: {timestamp: true, mode: true}
					},
					{
						...
						dest: '../luxalpabuilds/LuxAlpa/linux64/',
						flatten: true,
						filter: 'isFile',
						options: {timestamp: true, mode: true}
					},
		...
		nwjs: {
			options: {
				...
				macPlist: {
					CFBundleURLTypes: [{
						CFBundleURLName: 'LuxAlpa action',
						CFBundleURLSchemes: ['luxalpa']
		...
		compress: {
			linux32: {
				options: {
					archive: '../luxalpabuilds/luxalpa-linux32.zip'
				},
				expand: true,
				cwd: '../luxalpabuilds/luxalpa/linux32/',
				src: ['**/*'],
				dest: 'luxalpa-linux32/'
			},
			linux64: {
				options: {
					archive: '../luxalpabuilds/LuxAlpa-linux64.zip'
				},
				expand: true,
				cwd: '../luxalpabuilds/LuxAlpa/linux64/',
				src: ['**/*'],
				dest: 'LuxAlpa-linux64/'
			}
		},
		browserify: {
			dist: {
		...
		debian_package: {
			linux64: {
				files: [
					{
						expand: true,
						cwd: '../luxalpabuilds/luxalpa-test/linux64/',
						src: ['**/*'],
						dest: '/opt/luxalpa-test/'
					},
					//{expand: true, cwd: '../luxalpabuilds/luxalpa-test/linux64', src: ['luxalpa.desktop'], dest: '/usr/share/applications/luxalpa-test.desktop'}
				],
				options: {
					maintainer: {
						name: 'LuxAlpa',
						email: 'luxalpa@gmail.com'
					},
